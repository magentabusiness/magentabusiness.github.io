...
IoT Hub Team has already provisioned a valid certificate for [IoT Hub](https://iothub.magenta.at/){:target="_blank"}.
...
```bash
wget /docs/{{docsPrefix}}user-guide/resources/mqtt-over-ssl/ca-root.pem
```
{: .copy-code}

Now you may use the *ca-root.pem* to setup secure connection to IoT Hub Cloud and Access Token *YOUR_ACCESS_TOKEN* to authenticate the device to upload telemetry:
