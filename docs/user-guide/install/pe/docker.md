...
assignees:
- ashvayka
title: Installing IoT Hub using Docker (Linux or Mac OS)
description: Installing IoT Hub IoT Platform using Docker (Linux or Mac OS)
...
This guide will help you to install and start IoT Hub using Docker and Docker Compose on Linux or Mac OS. 
...
- `~/.mytbpe-data:/data`   - mounts the host's dir `~/.mytbpe-data` to IoT Hub data directory;
- `~/.mytbpe-data/db:/var/lib/postgresql/data`   - mounts the host's dir `~/.mytbpe-data/db` to Postgres data directory;
- `~/.mytbpe-logs:/var/log/thingsboard`   - mounts the host's dir `~/.mytbpe-logs` to IoT Hub logs directory;
