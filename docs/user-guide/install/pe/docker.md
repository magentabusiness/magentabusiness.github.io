...
assignees:
- ashvayka
title: Installing IoT Hub using Docker (Linux or MacOS)
description: Installing IoT Hub IoT Platform using Docker (Linux or MacOS)
...
This guide will help you to install and start IoT Hub using Docker and Docker Compose on Linux or MacOS. 
...
## Step 2. Choose IoT Hub queue service
...
- `tb-pe-license-data`   - name of the docker volume that stores the IoT Hub's license instance data file
...
Before you start IoT Hub, initialize the database schema and load built-in assets by running:   
...
**Note that web-reports will generate only if you access IoT Hub via external IP address or domain name.**
