---
layout: docwithnav
title: Connect IoT Hub to Datastax Astra DB 
description: Connect IoT Hub to Datastax Astra DB
hidetoc: "true"
---

This guide describes how to setup Datastax Astra DB cloud for your IoT Hub. This is an alternative having your own Cassandra cluster.
...
You need a `Database administrator` role to install the IoT Hub schema. Click `Create a token` on the `thingsboard` database.
...
### Step 5. Set connection parameters on IoT Hub

Here the environments you need to set to use your Astra DB with IoT Hub.

#### IoT Hub service

For IoT Hub as a local service add to the `/etc/thingsboard/conf/thingsboard.conf`
...
### Step 6. Run standard IoT Hub install

IoT Hub is already support install on Astra DB cloud.
...
### IoT Hub test run on Astra DB cloud

Here an example how to test your IoT Hub with your Astra DB using performance test tool.
To spin up the IoT Hub locally, use `docker-compose.yml` provided couple sections above.
