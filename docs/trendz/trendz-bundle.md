...
assignees:
- vparomskiy
title: Import Trendz bundle into IoT Hub
description: Import Trendz bundle into IoT Hub
...
All visualizations created in Trendz Analytics could be added on IoT Hub Dashboards. We created special `Trendz widget bundle` for IoT Hub - widgets collection that should be imported into IoT Hub `Widget Library`.
You can use them to add views from Trendz into IoT Hub dashboards and share analysis results with other users.

## Import Trendz widget bundle

### IoT Hub 3.4+ and Trendz 1.9+
You can import Trendz bundle to the IoT Hub via Trendz UI: 
...
* In case when IoT Hub already contains `Trendz bundle` but it is not up-to-date, the `Upload bundle` button would apply the latest changes.

### IoT Hub 3.3+ and Trendz 1.8+
Starting from IoT Hub 3.3 and Trendz 1.8 - Trendz widgets can be natively embedded into the IoT Hub dashboard.
Native Trendz widgets works much faster compared to original Trendz widgets that are based on iFrame. 

Add native Trendz library into IoT Hub extensions:
...
* Deploy library into IoT Hub extension directory
...
sudo chown iot hub:iot hub /usr/share/thingsboard/extensions/trendz-tb-lib-1.0.0-SNAPSHOT.jar
```

* Restart IoT Hub service to apply changes
...
* Login as Tenant Administrator into IoT Hub and go to **Widget Library**
...
* **Trendz View Static**- allow adding saved Trendz visualizations into IoT Hub dashboards
...
so they can create their own analysis using IoT Hub dashboard.
 
**Note:** If after importing Trendz Widget Bundle into IoT Hub, widgets do not work and white screen with error displayed - double-check
that correct bundle was imported. Widget API in IoT Hub v2.x and v3.x is different. Ensure that you downloaded bundle for 
the correct IoT Hub version.

## Troubleshooting

### IoT Hub 3.5+ blank widget with error
Starting from IoT Hub 3.5 we are using Angular 15 and link to library should be updated because standard link loads library that is based on Angluar 12, and it is not compatible with Angular 15.
...
If after importing Trendz Widget Bundle into IoT Hub, widgets do not work and white screen with error displayed - double-check
that correct bundle was imported. Widget API in IoT Hub v2.x and v3.x is different. Ensure that you downloaded bundle for
the correct IoT Hub version.

### HTTPS to HTTP links
If IoT Hub uses HTTPS and link to Trendz library uses http - you will see mixed content error in browser console and widget will not load. You should enable HTTPS for Trendz as well.
