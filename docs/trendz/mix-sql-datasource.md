...
assignees:
- vparomskiy
title: Combine data from IoT Hub and external databases
description: Combine data from IoT Hub and external SQL databases to compute metrics and analyze trends.
---

* TOC
{:toc}

IoT Hub is a reliable platform to manage your IoT entities – from devices and assets to customers and users. All these entities produce data: attributes and telemetry information, stored and organized neatly in the system.
...
To support such cases we updated Trendz to work with more than just IoT Hub data. You can now also pull data from external SQL databases into Trendz. 
This means you can combine data from IoT Hub with data from other systems and look at everything together in one place.

**For example:** if you have machine performance data in IoT Hub and maintenance logs or factory production rates in an external SQL database, you can merge and analyze them together in Trendz. 
...
Next step is to tell Trendz what data you want to pull from your external SQL database. Trendz has a concept of `Business Entity` that represents a group of IoT Hub entities, like device/asset/customer. 
Trendz also supports 'external' business entities that are not stored in IoT Hub, but in external SQL database. Note that business entity is not a single entity or row in the table, 
...
* Configure `Criteria` tab
  ...
  * `Entity type` = **EXTERNAL** - this is how Trendz knows that this business entity is not stored in IoT Hub
...
Finally, we need to tell Trendz how external Business Entity conencted with other entities in ThignsBoard. For example, if you have a table with `device_id` column, you can use it to connect external business entity with IoT Hub devices.

* Configure `Relations` tab
  * Click **Add relation** button
  * `Name` - set name of the relation
  * `Direction` - set to **FROM**
  * `Selected business entity` - select related IoT Hub business entity from the list
  * Enable created relation
* Save changes

## Security and data visibility
With the exciting capability to pull data from both IoT Hub and external SQL tables into Trendz, it's essential that we address an important aspect: security and data visibility. 
...
An external SQL table might have information for various entities in our system. When creating visualizations in Trendz that combine data from IoT Hub and these external tables, we prioritize ensuring that users only see what they're allowed to see.
IoT Hub already has a system in place where specific user permissions are set for each entity. Trendz taps into these permissions. 
Because Trendz understands how the external SQL table links to entities in IoT Hub, it can smartly filter data from the external sources.
...
Now, if certain users are only given access to specific machines in IoT Hub, when they view a Trendz visualization that combines machine data and maintenance history, they'll only see the maintenance details for the machines they have access to.
...
We already have `Equipment` entity in IoT Hub and `Maintenance` entity in external SQL database. And we alredy configured Trendz topology graph to connect these two entities via relation.
...
* Equipment serial number (from IoT Hub) set to `uniq` aggregation.
...
## Save incoming telemetry from IoT Hub into external SQL table
...
* Transform and Enrich Incoming Data: Before storing, enhance inciming telemetry event in IoT Hub Rule Engine to suit your reporting needs.
