...
All interactive visualizations created with the Trendz Analytics Platform could be shared with other users and embed on IoT Hub Dashboards 
...
* Open required IoT Hub Dashboard and press Edit button
...
By default, all Trendz visualizations use individual time range. However you can change this behavior and configure widget to toke time from IoT Hub Dashboard.
...
* JWT_TOKEN - IoT Hub JWT token that should be used to authenticate in the IoT Hub

## Blocked View problem

If HTTPS was not enabled for Trendz it is possible that visuals shared on 3rd party websites or on IoT Hub Dashboard 
would be blank. 

The problem is that most browser block mixed content requests: if IoT Hub use HTTPS and Trendz does not - browser will 
