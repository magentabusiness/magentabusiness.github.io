...
title: ESP8266 GPIO control over MQTT using IoT Hub
description: IoT Hub IoT Platform sample for ESP8266 GPIO control over MQTT
...
ESP8266 pushes data to IoT Hub server via MQTT protocol by using [PubSubClient](https://github.com/knolleary/pubsubclient) library for Arduino.
...
- thingsboardServer - IoT Hub HOST/IP address that is accessible from within your wifi network. Specify "iothub.magenta.at" if you are using [live demo](https://iothub.magenta.at/) server.
...
After the application is uploaded and started it will try to connect to IoT Hub node using mqtt client and upload current GPIOs state.
