...
description: IoT Hub IoT Platform sample for temperature data upload over MQTT using Raspberry Pi and DHT22 sensor.
...
Raspberry Pi push data to IoT Hub server via MQTT protocol by using [paho mqtt](https://eclipse.org/paho/clients/python/) python library.
...
If you are using [live demo](https://iothub.magenta.at/) server - [get the access token](/docs/user-guide/ui/devices/#manage-device-credentials) for pre-provisioned "DHT22 Demo Device".
