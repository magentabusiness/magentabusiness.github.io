---
layout: docwithnav
title: Publish MI-8 data to IoT Hub
description: Publish MI-8 data to IoT Hub guide
...
In this tutorial, we will configure FusionDAQ device to integrate it with IoT Hub.
...
Open your [IoT Hub](https://iothub.magenta.at/) in browser and sign in.
...
These are the fields which describe how to connect to IoT Hub or any other service.

MI-8 config.json file used in this example:

![image](/images/samples/fusion-daq/fusion-daq-config-json.png)

The **name** field is optional in this file. This **name** is displayed on the MI-8 OLED screen and is sent to IoT Hub as a device attribute. Best practice is for the name in config.json to match the name of the device in IoT Hub, but this doesn’t have to be the case. The name field isn’t used to associate telemetry data between the MI-8 and IoT Hub. It is only meant as an aid for the user.

The **push** field describes the connection to the IoT Hub servers. In this example we use HTTP POST requests (“mode”:”post”). MQTT is also supported by IoT Hub, but POST consumes less cellular data. The HTTP request URL is built up through the **server**, **port**, **use_ssl**, and **path/attributes_path** fields.

Things board telemetry (sensor data) requests are sent to https://iothub.magenta.at/:80/api/vi/{ACCESS_TOKEN}/telemetry
...
Set server to “**iothub.magenta.at**”. Username and password should remain empty.

Set **path** to “api/vi/{ACCESS_TOKEN}/telemetry” and set **attributes_path** to “api/vi/{ACCESS_TOKEN}/attributes”.
Replace **{ACCESS_TOKEN}** with the access token from the device credentials page in IoT Hub.
...
Set **port** equal to 80 and **use_ssl** equal to false to use an unsecured HTTP connection to push data to IoT Hub.
...
### Verify MI-8 Connection Within IoT Hub

At this point the MI-8 should be configured and ready to communicate with IoT Hub.
Return to your IoT Hub instance, then navigate to "Device groups" and then "All".
...
Now, navigate to the "Attributes" tab. Attributes are additional data sent from the MI-8 to IoT Hub that are more or less fixed and do not change such as the modem IMEI, the SIM ID (ICCID), and the MI-8 firmware version.
Attributes are only sent to IoT Hub once each time the MI-8 is powered on.
