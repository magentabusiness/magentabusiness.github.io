...
This guide contains step-by-step instruction how to connect your SODAQ NB-IoT boards to IoT Hub  through the T-Mobile NB IoT network. 
...
We expect you to have a very basic knowledge about IoT Hub, so we do recommend to complete the [Getting Started](/docs/getting-started-guides/helloworld/) guide.
 
## Integration overview

IoT Hub Platform Integrations feature allows pushing data from various platforms and connectivity solutions to IoT Hub. 
We will use "T-Mobile IoT CDP" platform integration to consume data from T-Mobile NB IoT Network and automatically register devices in IoT Hub.
Besides configuring the integration, we will also set up IoT Hub to decode incoming data, store it in the database, visualize on the dashboard and generate alarms based on configurable thresholds.
 ...
 * IoT Hub will automatically create a device with type "tracker" and name equal to IMEI;
...
Before we rush to T-Mobile IoT platform configuration, let's make sure IoT Hub is properly configured using simple cURL command.
