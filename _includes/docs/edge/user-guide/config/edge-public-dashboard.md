...
In **IoT Hub Edge**, **public dashboards** work similarly to how they do on the **IoT Hub Server**. 
...
```
* Even if the connection to the **IoT Hub Server** is lost, the **public dashboard** will continue to operate using real-time device data from the **Edge**.

* **Edge** will synchronize any updates or changes with the central **IoT Hub Server**, once connectivity is restored.
...
{% if docsPrefix == "pe/edge/" %}
**Dashboard groups** are managed on the **IoT Hub Server**. The dashboard group must first be created on the **IoT Hub Server** and then assigned to the **IoT Hub Edge** instance.
{% endif %}
