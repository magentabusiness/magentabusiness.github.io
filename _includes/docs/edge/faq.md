...
### What is IoT Hub Edge {{edgeName}}? 
{% if docsPrefix == "pe/edge/" or docsPrefix contains "pe/edge/" %}
<p><b>IoT Hub Edge Professional Edition (PE)</b> is a commercial version of <b>IoT Hub Edge</b> designed specifically for edge computing scenarios. It provides advanced features and enterprise-grade support for managing and analyzing IoT data at the edge while maintaining seamless synchronization with the <b>IoT Hub Server (Cloud or On-premise)</b>.</p>
...
{% else %}
<p><b>IoT Hub Edge Community Edition (CE)</b> is a free, open-source platform designed specifically for edge computing scenarios. It provides essential capabilities for managing and analyzing IoT data at the edge, while staying seamlessly synchronized with the <b>IoT Hub Server (Cloud or On-premise)</b>.</p>
<p>If you're new to edge computing, we recommend exploring <a href="/docs/{{docsPrefix}}getting-started-guides/what-is-edge/" target="_blank">What is Edge?</a> and the <a href="/docs/{{docsPrefix}}getting-started/" target="_blank">Getting Started</a> guides for more detailed information.</p>
{% endif %}


### Can I customize and modify the IoT Hub {{edgeName}}?
{% if docsPrefix == "pe/edge/" or docsPrefix contains "pe/edge/" %}
<p>No, the <b>IoT Hub {{edgeName}}</b> is a commercial project and can not be modified.</p>
{% else %}
...
### Does IoT Hub Edge support clustering? 

The **earlier versions** of **IoT Hub Edge** do not support clustering. It is designed to operate, process and analyze data locally before synchronizing with the central **IoT Hub Server**.
...
### Do I need an internet connection to use the IoT Hub Edge?
<p>No, you can run it completely offline if you need to. The only connection required is to the <b>IoT Hub Server</b> via <a href="https://grpc.io/" target="_blank">gRPC</a>.</p>
{% if docsPrefix == 'pe/edge/' %}
<p>However, <b>IoT Hub Edge</b> does utilize an HTTP(s) connection to the <b>IoT Hub Server</b> to verify the license.
...
The IoT Hub Server acts as a proxy for IoT Hub Edge to connect to the <a href="https://license.thingsboard.io/login" target="_blank">IoT Hub License Portal</a>.</p>
{% endif %}

{% if docsPrefix == 'pe/edge/' %}

### How will the license check be carried out if the connection to the IoT Hub Server is temporarily not available?
<p><b>IoT Hub Edge</b> can operate offline, without a connection to the <b>IoT Hub Server</b>, for up to <b>7 days</b>.</p>

{% endif %}

## Installation & Deployment 

### Does the IoT Hub Edge support multi-tenancy? 
{% if docsPrefix == 'pe/edge/' %}
<p>No, <b>IoT Hub Edge Professional Edition</b> does not support multi-tenancy. It is designed for a <b>single tenant</b> and <b>multiple customers</b>, but the customer hierarchy must be taken into account</p>
...
This means customers from the same hierarchy path can access the same IoT Hub Edge PE instance.</p>
<p>However, you cannot share a IoT Hub Edge between multiple tenants, and devices from multiple tenants cannot connect to a single <b>IoT Hub Edge</b>.
If this is required, you'll need to provision multiple <b>IoT Hub Edge</b> instances for each tenant.</p>
{% else %}
<p>No, <b>IoT Hub Edge Community Edition</b> does not support multi-tenancy. It is designed for a <b>single tenant</b> and a <b>single customer</b>, which means that you cannot share a single <b>IoT Hub Edge</b> instance between multiple tenants or customers, devices from different tenants cannot be connected to the same <b>IoT Hub Edge</b> instance.</p>
<p>If this is required, you'll need to provision multiple <b>IoT Hub Edge</b> instances for each tenant.</p>
{% endif %}


### Is there an official Docker image for IoT Hub Edge?
<p>Yes, official <b>Docker images</b> are available on <b>Docker Hub</b>.</p>

### Can I run IoT Hub Edge on Raspberry Pi or other edge devices? 
<p>Yes, <b>IoT Hub Edge</b> is specifically designed to run on a variety of Edge hardware platforms, including single-board computers like <b>Raspberry Pi</b> and other devices with sufficient processing power and memory.</p>

### Where does IoT Hub Edge store time-series data?
...
Since <b>IoT Hub Edge</b> is designed with remote locations with potentially low bandwidth connectivity in mind, we do not recommend connecting more than <b>1000</b> devices to a single edge.</p>
{% endif %}
...
### Can I integrate third-party systems with IoT Hub Edge?
<p>Yes, you can integrate the <b>IoT Hub Edge {{edgeName}}</b> with third-party systems through REST APIs.</p>
{% if docsPrefix == "pe/edge/" or docsPrefix contains "pe/edge/" %}
...
### What databases does the IoT Hub Edge support?
<p>The IoT Hub Edge supports pure SQL or a hybrid SQL + NoSQL (for telemetry storage) approach. For more details on database options, you can check <a href="/docs/{{peDocsPrefix}}reference/#sql-vs-nosql-vs-hybrid-database-approach" target="_blank">here</a>.</p>
...
### Is there a mobile app for IoT Hub Edge {{edgeName}}?
<p>No, there is no dedicated mobile app for <b>IoT Hub Edge {{edgeName}}</b>.</p>
<p>However, you can access and manage IoT Hub Edge through a web browser on any device, including mobile devices, by visiting the IoT Hub Edge dashboard (typically hosted on port 8080).</p>

### Does IoT Hub Edge {{edgeName}} support AI or machine learning integrations?
<p>Not natively, but you can incorporate AI or ML into your <b>IoT Hub Edge</b> deployment using custom development or third-party integrations.</p>

### How do I upgrade to the latest version of IoT Hub Edge?
<p>In order to upgrade to the latest version of <b>IoT Hub Edge</b>, please follow <a href="/docs/user-guide/install/{{docsPrefix}}upgrade-instructions/" target="_blank">these instructions</a>.</p>
...
<p><b>IoT Hub Edge</b> provides a simple API over common IoT protocols, so you can choose any client-side library you like, or even use your own. 
...
<p>Also, support can be provided by the <b>IoT Hub community</b></p>
{% else %}
...
<li><a href="/docs/{{docsPrefix}}" target="_blank">Documentation:</a> Comprehensive guides and API references available on the IoT Hub Documentation.</li>
</ul>
...
### Can I contribute to the development of IoT Hub Edge Community Edition?
