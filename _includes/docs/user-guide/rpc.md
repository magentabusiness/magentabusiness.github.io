...
IoT Hub RPC feature may be divided into two types based on the originator of the remote procedure execution:device-originated and server-originated RPC.
...
Before version 3.3, IoT Hub supported **lightweight** RPC only. 
...
They lived in memory of the server, assuming that if server dies, the dashboard widget will send the same request to other IoT Hub server in the cluster.
...
Since version 3.3, IoT Hub provides support of [**persistent**](#persistent-rpc) RPC calls.
...
- **http(s)://host:port** is your IoT Hub server base URL. For example, [https://iothub.magenta.at](https://iothub.magenta.at)
