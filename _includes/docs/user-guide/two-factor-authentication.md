...
Two-factor authentication is a state-of-the-art approach designed to provide an extra security layer. With 2FA, even though someone knows your password your IoT Hub account is safe against malicious access.
...
## Two-factor authentication options available in IoT Hub
...
Follow the steps below to enable two-factor authentication for your IoT Hub instance:
{% if docsPrefix == null %}
- Log in to IoT Hub as the system administrator;
{% endif %}
{% if docsPrefix == "pe/" %}
- Log in to IoT Hub as the system administrator or tenant administrator;
{% endif %}
{% if (docsPrefix == "paas/") or (docsPrefix == "paas/eu/") %}
- Log in to IoT Hub as the tenant administrator;
{% endif %}
