...
To avoid this painful and mundane activity, since IoT Hub 3.2, you can specify a custom Rule Chain for your devices.
...
{% if (docsPrefix == null) or (docsPrefix == "pe/") %}
```bash
mosquitto_pub -h 'iothub.magenta.at' -i 'c1' -u 't1' -P 'secret' -t 'v1/devices/me/telemetry' -m '{"humidity": 10.3}'
```
{: .copy-code}
{% endif %}
{% if docsPrefix contains "paas/" %}
...
{% if (docsPrefix == null) or (docsPrefix == "pe/") %}
```bash
mosquitto_pub -h 'iothub.magenta.at' -i 'c1' -u 't1' -P 'secret' -t 'v1/devices/me/attributes' -m '{"firmwareVersion": "1.3"}'
```
{: .copy-code}
{% endif %}
{% if docsPrefix contains "paas/" %}
...
The current version of the IoT Hub platform supports customizable proto schemas for [telemetry upload](/docs/{{docsPrefix}}reference/mqtt-api/#telemetry-upload-api) 
...
The current version of the IoT Hub platform supports customizable proto schemas for [telemetry upload](/docs/{{docsPrefix}}reference/coap-api/#telemetry-upload-api) 
...
The current version of the IoT Hub platform supports integration with next Efento NB-IoT sensors: 
