...
IoT Hub has been run in production by numerous companies in both [monolithic](/docs/{{docsPrefix}}reference/monolithic/)
and [microservices](/docs/{{docsPrefix}}reference/msa/) deployment modes.
This article describes the performance of a single IoT Hub server in the most popular usage scenarios.
It is helpful to understand how IoT Hub scales vertically (monolith) before describing how it scales horizontally (cluster mode).

## Test methodology

For simplicity, we have deployed a single IoT Hub instance with all related third-party components in a docker-compose environment on a single EC2 instance.
...
IoT Hub stored all the time-series data in the database.
IoT Hub also processed the data using the [alarm rules](/docs/{{docsPrefix}}user-guide/device-profiles/#alarm-rules) to create alarms if the battery level is low.
...
<details markdown="1">
<summary>
Setup the IoT Hub instance on AWS EC2
</summary>
...
Previous section demonstrates **successful** IoT Hub deployment designed up to 3000 data points/sec.
...
IoT Hub docker compose with no change with the previous section.
...
Kafka CPU and disk IO overhead are tiny relative to Postgres and IoT Hub CPU consumption.
...
Here is the docker-compose file to set up _IoT Hub + Postgresql + Zookeeper + Kafka_ on AWS EC2 instance based on the [instruction](https://github.com/thingsboard/performance-tests).
...
IoT Hub + Postgres + **Kafka** - is a reliable solution to survive peak loads.
...
In a real production, the IoT Hub may serve all kind of user requests, run custom rule chains and supply the web services for all fancy dashboards.
...
All systems have to run maintenance in background, so it is completely fine to have those chart for IoT Hub monolith deployment.
...
Finally, let's check the JVM state on each IoT Hub, Zookeeper, Kafka and Cassandra
Let's forward JMX port with SSH to connect and monitor all Java applications presented.

```bash
ssh -L 9999:127.0.0.1:9999 -L 1099:127.0.0.1:1099 -L 9199:127.0.0.1:9199 -L 7199:127.0.0.1:7199 IoT Hub 
```
{: .copy-code}
...
Here the JMX monitoring for IoT Hub, Kafka, Zookeeper, Cassandra. The system is stable.

{% include images-gallery.html imageCollection="cassandra-25k-10k-30k-jmx" %}

**Lessons learned**

**Cassandra** is essential for massive telemetry flow. Peak load is handled with Kafka queue. Bundling Kafka + PostgreSQL + Cassandra is a **top IoT Hub deployment**.
...
It is a good idea to start with Cassandra from the very beginning of your IoT Hub instance and maintain the same stack for the entire project lifetime.
...
System can be scaled up vertically up to 50-100%. For significant horizontal scaling, please, consider to set up a IoT Hub cluster.
...
Here the docker-compose with IoT Hub + Postgresql + Zookeeper + Kafka + **Cassandra**
...
In addition to conclusions we made for [Scenario C](#scenario-c), we observe stability of a single IoT Hub instance handling 100K parallel MQTT connections.
...
* All device connected. Java JMX. VisualVM -> IoT Hub -> MBeans -> java.lang -> OpenFileDescriptorCount -> more than 100000
...
Thanks to the IoT Hub community that creates issues and shares performance solutions around the time.
...
It was IoT Hub 3.3.3 version. We will work on performance improvements in 3.4.x
