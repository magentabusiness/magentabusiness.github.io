...
This section covers how to provision your first LwM2M device in IoT Hub. We will use [Eclipse Wakaama](https://github.com/eclipse/wakaama#test-client-example){:target="_blank"} test client to simulate LwM2M device.
...
IoT Hub provides full support for both an LwM2M Server and a Bootstrap Server, with communication over:
- **Plain UDP**
- **DTLS** (secure transport over UDP)

As a platform user, you are able to provision LwM2M devices and define the mapping between the LwM2M resources and IoT Hub device [attributes](/docs/{{docsPrefix}}user-guide/attributes/){:target="_blank"} and [telemetry](/docs/{{docsPrefix}}user-guide/telemetry/){:target="_blank"} (time series data).
...
To trigger a **reboot** on the device using resource */3/0/4*, send the following RPC command to IoT Hub:
    ...
    "
      LwM2mObjectInstance 
        [
          id=0, resources=
            {
              0=LwM2mSingleResource [id=0, value=IoT Hub Test Device, type=STRING], 
              1=LwM2mSingleResource [id=1, value=Model 500, type=STRING], 
              2=LwM2mSingleResource [id=2, value=TH-500-000-0001, type=STRING], 
              3=LwM2mSingleResource [id=3, value=TestIoT Hub@TestMore1024_2.04, type=STRING], 
