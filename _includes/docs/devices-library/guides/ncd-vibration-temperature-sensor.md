...
This tutorial provides a comprehensive walkthrough of the essential configuration steps, including connecting NCD devices, registering them on the platform, setting up a IoT Hub account, adding and linking a new gateway, and creating a dashboard to visualize real-time sensor data effectively.
...
The first step is to [sign in using your IoT Hub account](https://iothub.magenta.at/){:target="_blank"}. 
...
<br>
**ncd-to-thingsboard**: This MQTT output node establishes a connection to the MQTT Broker using the required credentials to connect to IoT Hub, allowing the data generated by the vibration sensor to be published to the IoT Hub.
...
This tutorial explained the basic, step-by-step process of integrating the NCD Industrial IoT Wireless Vibration and Temperature Sensor with IoT Hub, enabling you to manage and visualize the vibration and temperature variables sent by the NCD sensor.
