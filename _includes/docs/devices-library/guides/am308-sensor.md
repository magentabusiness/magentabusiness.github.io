...
Using Milesight LoRaWAN® gateway and IoT Hub, users can manage all sensor data remotely and visually.
...
Since this device can only be operated using a LoRaWAN® gateway, we must first connect it to a network server that has an integration configured with IoT Hub.  
Afterward, it can be provisioned to IoT Hub.
