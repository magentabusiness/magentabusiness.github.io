...
{% assign deviceProfileCreationPE = '
    ...
    ===
        ...
        title: Once the profile is ready you will see it on the list in the "Device profiles" section. If you plan to use your IoT Hub instance only with Efento sensors you can set the profile as default.
'
%}
...
In order to save a data from the sensor we should also create a device on IoT Hub.  
...
{% assign deviceAddingPE = '
    ...
    ===
        ...
        title: Leave the credential type as "Access token". Set the new access token, which will be used by a sensor to validate in the IoT Hub platform. Note that the token must be unique for each sensor. Configuration of the token is optional. Click "Add" button to confirm adding new device;
    ===
        image: /images/devices-library/ready-to-go-devices/wireless-open-close-sensor/create-device-4-pe.png,
        title: A window will open where you can check the device&#39;s connection to IoT Hub. This step is optional. Close this window;
    ===
