...
{% assign gatewayConfiguration = '
    ...
    ===
        ...
        title: Activate the Cloud control and configure all the parameters to connect the device to the specific IoT Hub platform via MQTT.
    ===
...
|**Access Token**| Indicate the Access token previously copied in IoT Hub. |
...
To verify that the device is connected correctly to IoT Hub, go to the **Device groups** menu -> **All** devices, select your device.  
...
The EXXN IoT Gateway will connect to IoT Hub using the MQTT API.  
We have previously covered how to configure the device to connect to IoT Hub.  
Now, we will show the steps to configure the device in IoT Hub in order to monitor data and manage the device.  
