In order to get the state of the device from IoT Hub during booting we have [functionality](/docs/{{page.docsPrefix}}reference/mqtt-api/#request-attribute-values-from-the-server) to do this in the code.  
...
...
``` 
In order for our callbacks to receive the data, we have to send a request to IoT Hub.  