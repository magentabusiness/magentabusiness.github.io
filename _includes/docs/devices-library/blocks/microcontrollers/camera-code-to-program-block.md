...
Now it’s time to program the board to connect to IoT Hub.  
...
// See https://docs.iothub.magenta.at/docs/{{page.docsPrefix}}getting-started-guides/helloworld/
// to understand how to obtain an access token
constexpr char TOKEN[] = "YOUR_ACCESS_TOKEN";

// IoT Hub we want to establish a connection too
...
// Maximum amount of attributs we can request or subscribe, has to be set both in the IoT Hub template list and Attribute_Request_Callback template list
...
// Initialize IoT Hub instance with the maximum needed buffer size, stack size and the apis we want to use
...
Data, send by this device may require increasing of the allowed message size for MQTT on **your IoT Hub instance**.  
...
Don’t forget to replace placeholders with your real WiFi network SSID, password, IoT Hub device access token.
{% endcapture %}
...
| THINGSBOARD_SERVER | **{{hostName}}**             | Your IoT Hub host or ip address.                                                                                                         |
| THINGSBOARD_PORT | **1883U**                    | IoT Hub server MQTT port. Can be default for this guide.                                                                                 |
