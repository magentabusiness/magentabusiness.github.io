...
<br>
Here you will see information about the new device. As well as the telemetry which we sent from the device to the IoT Hub.
...
**NOTE**
<br>
If the "Allow create devices or assets" checkbox is unchecked, when sending a message to the IoT Hub with any parameters of the device (or asset), if such a device (asset) does not exist, then device (asset) will not be created.
{% endcapture %}
...
<br>
Send the uplink message again. We will receive a response from the IoT Hub in the terminal:
