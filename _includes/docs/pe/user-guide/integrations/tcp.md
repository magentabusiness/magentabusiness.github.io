...
An uplink converter that is a script for parsing and transforming the data received by TCP integration to a format that IoT Hub can consume.
...
For sending downlink messages from the IoT Hub to the device, we need to define a downlink converter.
