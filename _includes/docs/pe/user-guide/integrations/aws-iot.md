...
AWS IoT Integration allows to stream data from AWS IoT Backend to IoT Hub and converts device payloads
to the IoT Hub format. AWS IoT will be primarily responsible for receiving all messages (as a broker -
...
perform a few settings. To establish a correct and secure connection between the service and IoT Hub, you need to create Policies, devices, and certificates for them.
...
them in IoT Hub. Click on the “plus” and on “Create new converter”. To view the events, enable Debug.
...
To do this, you need to open the Rule Chain used for the device (in our case, the default Root Rule Chain), then add an [integration downlink node](/docs/{{docsPrefix}}user-guide/rule-engine-2-0/action-nodes/#integration-downlink-node), for link condition set the Attributes Updated.

{% include images-gallery.html imageCollection="downlink_2-3" %}

To subscribe to a topic for receive messages from IoT Hub, use the **AWS MQTT test client**.
