...
The job of IoT Hub Integration is to provide secure and reliable API bridge between core platform features (telemetry collection, attributes and RPC calls) and specific third-party platform APIs.    

### How it works?

At the moment IoT Hub supports various integration protocols. Most popular are HTTP, MQTT and OPC-UA. 
...
Once message arrives from External Platform to IoT Hub it passes validation according to platform specific payload format and security rules. 
Once message is validated IoT Hub Integration invokes assigned [**Uplink Data Converter**](/docs/{{peDocsPrefix}}user-guide/integrations/#uplink-data-converter) to extract sub-set of meaningful information out of the incoming message.
The message is basically transformed from device and platform specific payload to the format that IoT Hub uses.

Since IoT Hub v2.0, Rule Engine is also able to push Downlink messages to the integrations. The example of such message may be:
...
Once message is pushed by the rule engine, IoT Hub invokes assigned [**Downlink Data Converter**](/docs/{{peDocsPrefix}}user-guide/integrations/#downlink-data-converter) and transforms the rule engine message to the specific data format that is used by the Integration.
...
The main function of **Uplink Data Converter** is to parse payload of the incoming message and transform it to format that IoT Hub uses.
...
Starting version 2.4.2, IoT Hub also supports **assetName** and **assetType** instead of deviceName and deviceType.
{% endcapture %}
...
**NOTE**:
<br>
Starting version 2.4.2, IoT Hub also support optional **customerName** and **groupName**.
...
Starting from version 3.9, IoT Hub stores all debug events for the first 15 minutes. After that, only failure events are retained. These settings can be combined or completely disabled.

### Platform Integrations vs IoT Gateway

Experienced IoT Hub users may notice that functionality of Integrations feature partially overlap with functionality of [IoT Gateway](/docs/iot-gateway/what-is-iot-gateway/).
However, there are key differences between these two systems/features:
  ...
  - IoT Gateway is designed to support < 1000 devices, while Integrations are designed for high throughput, scalability and cluster deployments as part of IoT Hub server.
