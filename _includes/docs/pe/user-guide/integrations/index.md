...
The message is basically transformed from device and platform specific payload to the format that IoT Hub uses. 6
...
Once message is pushed by the rule engine, IoT Hub invokes assigned [**Downlink data converter**](#downlink-data-converter) and transforms the rule engine message to the specific data format that is used by the Integration.
...
{% capture difference %}
**NOTE**: Starting version 2.4.2, IoT Hub also supports **assetName** and **assetType** instead of deviceName and deviceType.
{% endcapture %}
...
Therefore, starting from version 3.9, IoT Hub stores all debug events only during the first 15 minutes. After that, only failure events are retained.
