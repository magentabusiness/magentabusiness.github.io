...
The **IoT Hub Scheduler** is a feature that allows users to automate periodic tasks, such as telemetry and attribute updates, RPC commands, and rule chain triggers.

The **Scheduler** feature for **Edge** is designed similarly to that of the **Platform**. 
For a comprehensive understanding, please refer to the **IoT Hub Platform** [Scheduler documentation](/docs/pe/user-guide/scheduler/){: target="_blank"}.

**Scheduler events** cannot be created or modified directly from **IoT Hub Edge**, all scheduling must be configured in **IoT Hub Platform** and then assigned to the **Edge** instance.
...
### Scheduled Irrigation Management Using IoT Hub Edge (Use Case)
...
Since irrigation systems are often deployed in remote agricultural fields, the **IoT Hub Edge** instance will run locally to ensure uninterrupted operation, even in cases of network disruptions.
...
* Monitor critical parameters in real-time, such as:
  ...
  * Irrigation duration.
* Operate locally on **IoT Hub Edge** to avoid reliance on constant internet connectivity.

To install the solution template, log in to the **IoT Hub Platform** and go to the **Solution templates** section. 
...
Since **rule chains** and **scheduler events** cannot be created directly on the **Edge**, all configurations are first completed on the **IoT Hub Platform** and then synchronized with the **Edge** instance.
