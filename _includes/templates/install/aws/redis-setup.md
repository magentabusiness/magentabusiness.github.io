You'll need to set up [Amazon ElastiCache (Redis)](https://aws.amazon.com/elasticache/redis/). IoT Hub uses cache to improve performance and avoid frequent DB reads.
...
* Make sure your Redis cluster is accessible from the IoT Hub cluster. The easiest way to achieve this is to deploy the Redis cluster in the same VPC. We also recommend to use private subnets. Use "eksctl-thingsboard-cluster-ClusterSharedNodeSecurityGroup-*" security group;
