...
```bash
http(s)://iothub.magenta.at/api/v1/$ACCESS_TOKEN/attributes/updates
```
{: .copy-code}
...
Execute the command:
```bash
curl -v -X GET http(s)://iothub.magenta.at/api/v1/$ACCESS_TOKEN/attributes/updates?timeout=20000
```
{: .copy-code}
...
```bash
http(s)://iothub.magenta.at/api/v1/$ACCESS_TOKEN/attributes
```
{: .copy-code}
