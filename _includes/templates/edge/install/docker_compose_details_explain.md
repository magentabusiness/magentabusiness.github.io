...
Where:    
- **restart: always:** IoT Hub Edge automatically starts on system reboot or after a failure.
...
{% if docsPrefix == 'pe/edge/' %}
- **thingsboard/tb-edge-pe:{{ site.release.pe_edge_full_ver }}:** The IoT Hub Edge PE Docker image.
{% else %}
- **thingsboard/tb-edge:{{ site.release.edge_full_ver }}:** The IoT Hub Edge Docker image.
{% endif %}
- **CLOUD_ROUTING_KEY:** Enter the actual Edge key.
- **CLOUD_ROUTING_SECRET:** Enter the actual Edge secret.
- **CLOUD_RPC_HOST:** he IP address or hostname of the machine running the IoT Hub platform.
{% if docsPrefix == 'pe/edge/' %}
- **CLOUD_RPC_SSL_ENABLED:** Defines whether SSL is used for the connection between Edge and the IoT Hub server. Use _true_ to enable SSL, or _false_ to disable it.
{% endif %}
...
{% if docsPrefix == 'pe/edge/' %}
* Use **iothub.magenta.at** if you are connecting Edge to the [**ThingsBoard Cloud**](https://iothub.magenta.at/signup){: target="_blank"}.

**NOTE**: **iothub.magenta.at** uses SSL protocol for edge communication.
Please change **CLOUD_RPC_SSL_ENABLED** to **true** as well.
{% else %}
* Use **iothub.magenta.at** if you are connecting Edge to the [**ThingsBoard Live Demo**](https://iothub.magenta.at/signup){: target="_blank"} for evaluation.
{% endif %}
...
If **IoT Hub Edge** is set to run on the **same machine** where the **{{appPrefix}}** server is operating, you need to update port configuration to prevent port collision between the IoT Hub server and IoT Hub Edge.
