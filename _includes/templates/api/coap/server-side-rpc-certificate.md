...
{% if docsPrefix == null or docsPrefix == "pe/" %}
- Subscribe to RPC commands from the server using the command below. To do this, in the first terminal window send GET request with observe flag. Don't forget to replace <code>iothub.magenta.at</code> with your host:

```shell
coap-client -m get coap://iothub.magenta.at/api/v1/rpc -s 100 -B 100
```
{: .copy-code}
{% endif %}
{% if docsPrefix == "paas/" or docsPrefix == "paas/eu/"%}
...
{% if docsPrefix == null or docsPrefix == "pe/" %}
```shell
coap-client -f rpc-response.json -m post coap://iothub.magenta.at/api/v1/rpc/1
```
{: .copy-code}
{% endif %}
{% if docsPrefix == "paas/" or docsPrefix == "paas/eu/"%}
